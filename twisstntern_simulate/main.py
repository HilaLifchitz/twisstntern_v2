...
def parse_downsampleKB_arg(downsampleKB_str):
    ...
    if "+" in downsampleKB_str:
        ...
        if len(parts) != 2:
            raise ValueError(
                f"Invalid downsampleKB format '{downsampleKB_str}': expected 'Nkb+ikb'"
            )
        ...
        if not N_part.endswith("kb"):
            raise ValueError(
                f"Invalid downsampleKB format '{downsampleKB_str}': sampling interval must end with 'kb'"
            )
        try:
            N = int(N_part[:-2])
            if N <= 0:
        ...
        if i_bp >= N_bp:
            raise ValueError(
                f"Starting position ({i_part} = {i_bp} bp) must be less than sampling interval ({N}kb = {N_bp} bp)"
            )

        return N, i_bp

    raise ValueError(
        f"Invalid downsampleKB format '{downsampleKB_str}': expected 'Nkb' or 'Nkb+ikb'"
    )
...
def parse_position_with_units(position_str):
    ...
    if position_str.endswith("kb"):
        ...
        raise ValueError(
            f"Invalid position format '{position_str}': expected 'Nkb', 'Nmb', 'Ngb', or 'N' (base pairs)"
        )
