            ...
            if "." in key_path:
                ...
                if parts[0] == "migration":
                    ...
                    if hasattr(config, "migration") and config.migration:
                        ...
                        applied_overrides[f"migration.{migration_key}"] = (
                            f"{old_value} -> {value}"
                        )
                        logger.info(
                            f"Override applied: migration.{migration_key}: {old_value} -> {value}"
                        )
                    else:
                        raise ValueError(
                            f"Migration configuration not found for override: {override_str}"
                        )
                    ...
                    if len(parts) >= 3:
                        ...
                        for pop in config.populations:
                            if pop.name == pop_name:
                                ...
                                applied_overrides[
                                    f"populations.{pop_name}.{pop_attr}"
                                ] = f"{old_value} -> {value}"
                                logger.info(
                                    f"Override applied: populations.{pop_name}.{pop_attr}: {old_value} -> {value}"
                                )
                                break
                        else:
                            raise ValueError(
                                f"Population '{pop_name}' not found for override: {override_str}"
                            )
                    else:
                        raise ValueError(
                            f"Invalid population override format: {override_str}"
                        )
                else:
                ...
                if key_path == "samplesize":
                    ...
                    for pop in config.populations:
                        if not pop.is_ancestral and pop.sample_size is not None:
                            ...
                            applied_overrides[f"samplesize.{pop.name}"] = (
                                f"{old_size} -> {value}"
                            )
...
def run_pipeline(
    ...
    try:
        ...
        simulation_results = run_simulation(
            config, output_dir=output_dir, mode_override=config.simulation_mode
        )
        ...
        if hasattr(config, "population_labels") and config.population_labels:
            ...
            populations_with_samples = [
                pop
                for pop in config.populations
                if not pop.is_ancestral and pop.sample_size and pop.sample_size > 0
            ]
        ...
        if downsample_N is not None and downsample_N > 1:
            ...
            logger.info(
                f"Downsampling: keeping every {downsample_N}th data point starting from index {downsample_i}."
            )
            ...
            topology_weights_downsampled = topology_weights.iloc[
                indices, :
            ].reset_index(drop=True)

            logger.info(
                f"Downsampled to every {downsample_N}th data point starting from index {downsample_i}. {len(topology_weights_downsampled)} remain."
            )
            downsample_mode = (
                f"every {downsample_N}th starting from index {downsample_i}"
            )
            ...
            if "position" in topology_weights.columns:
                ...
                for pos in target_positions:
                    candidates = topology_weights.index[
                        topology_weights["position"] >= pos
                    ]
                    if len(candidates) > 0:
                        idx = candidates[0]
                        if idx != last_idx:
                ...
                topology_weights_downsampled = topology_weights.loc[
                    selected_indices
                ].reset_index(drop=True)
                ...
                logger.info(
                    f"Downsampled to one data point every {downsample_kb} kb starting from {start_display}. {len(topology_weights_downsampled)} remain."
                )
                downsample_mode = (
                    f"every {downsample_kb} kb starting from {start_display}"
                )
            else:
                logger.warning(
                    "KB downsampling requested but no 'position' column found in topology weights. Skipping KB downsampling."
                )

        n_after_downsample = len(topology_weights_downsampled)
        if downsample_mode:
            logger.info(
                f"Number of data points after downsampling ({downsample_mode}): {n_after_downsample}"
            )
        else:
            logger.info(
                f"No downsampling applied. Data points used: {n_after_downsample}"
            )
        ...
        if mode == "chromosome":
            logger.info(
                f"Total trees computed: {n_total_trees} (n_right + n_left = {n_trees_used})"
            )

            # Always explain T2=T3 filtering (even when 0)
            n_filtered = n_total_trees - n_trees_used
            logger.info(
                f"Note: {n_filtered} trees were filtered out (trees where T2 = T3, which fall exactly on the y-axis in ternary space)"
            )

        elif mode == "locus":
            logger.info(
                f"n_loci simulated: {config.n_loci}, total trees analyzed: {n_total_trees} (n_right + n_left = {n_trees_used})"
            )
            ...
            if n_total_trees < config.n_loci:
                ...
                logger.info(
                    f"Additional note: {n_loci_filtered} loci were filtered out during processing."
                )
        ...
        return triangles_results, fundamental_results, weights_file
