...
1. Population Definitions:
   - List of populations with their properties:
     ...
     * "growth_rate": Population growth rate (default: 0.0) OPTIONAL
...
2. Split Times:
   - List of population split events with:
     ...
     * "derived_pop1": Name of the first derived population
     * "derived_pop2": Name of the second derived population
     * "ancestral_pop": Name of the ancestral population

3. Migration Rates:
   - Dictionary of migration rates between populations
   - Format: "source>dest": rate
...
1. Simulation Mode:
   - "simulation_mode": Choose between "locus", "chromosome", or "both"
   - "locus" mode: Simulates independent non-recombining loci
   - "chromosome" mode: Simulates a recombining chromosome
   - "both" mode: Runs both types of simulations

2. Common Parameters:
   - "mutation_rate": Mutation rate per base per generation (optional)
   - "seed": Random seed for reproducibility (optional)

3. Locus Mode Parameters (required if mode is "locus" or "both"):
   - "n_loci": Number of loci / windows to simulate
   - "locus_length": Length of each locus in base pairs

4. Chromosome Mode Parameters (required if mode is "chromosome" or "both"):
...
class Config:
    ...
    3. Both modes: Runs both types of simulations
    
    The configuration file should be in YAML format and include all necessary
    parameters for the chosen simulation mode(s).
    """
    ...
    def __init__(self, config_file: str):
        ...
        # Load ploidy
        self.ploidy = self.config.get('ploidy', 2)  # Default to diploid if not specified
        ...
        for pop in self.config.get('populations', []):
            ...
            self.populations.append(Population(
                name=pop['name'],
                Ne=pop['Ne'],
                growth_rate=pop.get('growth_rate', 0.0),  # Optional, defaults to 0.0
        ...
        if self.simulation_mode in ['locus', 'both']:
        ...
        if self.simulation_mode in ['chromosome', 'both']:
    ...
    def _validate(self):
        ...
        if self.simulation_mode not in ['locus', 'chromosome', 'both']:
            raise ValueError("simulation_mode must be 'locus', 'chromosome', or 'both'")
        ...
        for split in self.splits:
            ...
            if split.derived_pop1 not in [p.name for p in self.populations]:
                raise ValueError(f"First derived population {split.derived_pop1} not found")
            if split.derived_pop2 not in [p.name for p in self.populations]:
                raise ValueError(f"Second derived population {split.derived_pop2} not found")
            if split.ancestral_pop not in [p.name for p in self.populations]:
                raise ValueError(f"Ancestral population {split.ancestral_pop} not found")
    ...
    def _validate_population(self, pop: Dict[str, Any]) -> None:
        ...
        if "growth_rate" in pop:
            try:
                ...
                if growth_rate < 0:
                    raise ValueError(f"Population {pop['name']} growth_rate must be non-negative")
                pop["growth_rate"] = growth_rate  # Convert to float
            except (ValueError, TypeError):
                raise ValueError(f"Population {pop['name']} growth_rate must be a number")
        ...
        if "sample_size" in pop and pop["sample_size"] is not None:
            if not isinstance(pop["sample_size"], int) or pop["sample_size"] <= 0:
                raise ValueError(f"Population {pop['name']} sample_size must be a positive integer")
            if pop['name'] in ['p12', 'p123', 'ANC']:
                raise ValueError(f"Sample size should not be specified for ancestral population {pop['name']}")
            else:
        ...
        if "label" in pop:
            ...
            pop['label'] = pop['name']  # Default label to population name if not provided
        
        # Validate is_ancestral
        if "is_ancestral" in pop:
            if not isinstance(pop["is_ancestral"], bool):
                raise ValueError(f"Population {pop['name']} is_ancestral must be a boolean")
        else:
            pop['is_ancestral'] = False  # Default to False if not provided
        
        # Validate growth_rate only for extant populations
        if pop['name'] not in ['p12', 'p123', 'ANC'] and pop.get('growth_rate') is not None:
