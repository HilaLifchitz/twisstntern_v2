...
# For a given triangle T_i(tau_i, x) = 0, find the range of x values on each side.
def T2_lim(t2):
    """
...
# Find the limits at the T2 T3 boundary which goes from t = 0.5, x= -0.5 to t=0, y=0.5
def T3_lim_symm(t3):
    """
    Calculate symmetric x-limits for isocline T3 = t3 relative to the triangular boundary.
    Used for symmetric analysis across the T2-T3 boundary.
    """
    x_L = 0.5 * (2 * t3 - 1)  # from T3(t3, x) = 0
    x_R = 0.5 * t3  # from T3(t3, x) = T2(0, x)
...
def dump_data(file_path, logger=None):
    ...
    if missing_cols:
        error_msg = f"Missing required columns: {missing_cols}. Available columns: {list(data.columns)}"
        if logger:
            logger.error(error_msg)
        raise ValueError(error_msg)
    ...
    for col in required_cols:
        if not pd.api.types.is_numeric_dtype(data[col]):
            try:
                ...
                if logger:
                    logger.warning(f"Converted column {col} to numeric")
            except Exception:
                error_msg = f"Column {col} contains non-numeric data that cannot be converted"
                if logger:
                    logger.error(error_msg)
                raise ValueError(error_msg)
    ...
    if nan_count > 0:
        if logger:
            logger.warning(f"Found {nan_count} NaN values in ternary columns, removing affected rows")
        data = data.dropna(subset=required_cols)
        
    # Basic validation: check if values are approximately normalized
    row_sums = data[['T1', 'T2', 'T3']].sum(axis=1)
    deviation = np.abs(row_sums - 1.0)
    
    # Warn if many rows deviate significantly from normalization constraint
    problematic_rows = (deviation > 0.01).sum()
    if problematic_rows > 0:
        percentage = 100 * problematic_rows / len(data)
        if logger:
            logger.warning(
                f"{problematic_rows} rows ({percentage:.1f}%) deviate from normalization constraint (T1+T2+T3=1) by >0.01"
            )
    
    # Basic range validation (should be between 0 and 1)
    for col in required_cols:
        out_of_range = ((data[col] < 0) | (data[col] > 1)).sum()
        if out_of_range > 0:
            if logger:
                logger.warning(f"Column {col}: {out_of_range} values outside [0,1] range")
    
    if logger:
        logger.info(f"Data validation complete. Final shape: {data.shape}")
        logger.debug(f"Data summary:\n{data[required_cols].describe()}")
    
    return data


# The following functions are related to the analysis and statistical computations.
